...
// The 8B graph is split into parts (< 2 GB each) that run in separate Web Workers (one onnxruntime-web instance each: 4 GB wasm heap limit, ArrayBuffer size limits).
import { confirmDownload } from './models.js';

const params = new URLSearchParams(location.search);
export const ENCODER = {
  id: 'llama3-llm2vec-int4', label: 'Llama-3-8B LLM2Vec text encoder (INT4 ONNX, in parts)', size_mb: 4980, seq: 64,
  hf: 'https://huggingface.co/sprited/ardy-web-onnx/resolve/main/text-encoder/',
};
function base() { const p = params.get('encoder'); if (p === 'local') return new URL('models/encoder-sprited/', location.href).href; if (p && p !== 'hf') return p.endsWith('/') ? p : p + '/'; return ENCODER.hf; }
...
export async function loadTextEncoder(onProgress) {
  ...
  ENCODER.size_mb = Math.round(manifest.parts.reduce((a, p) => a + (p.size_bytes || 0), 0) / 1e6) || ENCODER.size_mb;
...
/** Prompt -> Float32Array(4096) text feature for the ARDY window graph: the parts run in order, each fed the
 *  token inputs plus the previous part's outputs (the hidden state). */
export async function encodePrompt(prompt) {
  ...
  const t0 = performance.now(), partMs = []; let outputs = null;
  for (const part of manifest.parts) { const r = await runWorker(part.name, feeds); partMs.push(Math.round(r.ms)); outputs = r.outputs; feeds = { ...feeds, ...outputs }; }
  return { feat: Float32Array.from(outputs.text_embedding.data), ms: performance.now() - t0, partMs };
}
