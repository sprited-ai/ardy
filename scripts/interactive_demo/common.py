...
class GuiElements:
    ...
    gui_generation_backend: viser.GuiDropdownHandle[str]
    gui_compute_label: viser.GuiMarkdownHandle
...
class ClientSession:
    ...
    # (start_frame, kind, label): which backend computed the frames from start_frame on ("Compute" label)
    compute_schedule: list = field(default_factory=list)
    now_compute_text: Optional[str] = None
